...
def test_that_loading_an_unparseable_snapshot_returns_none_and_logs_warning(
    tmp_path, caplog
):
    path = tmp_path / "snapshot_0.json"
    ...
    with caplog.at_level(logging.WARNING):
        assert load_status_snapshot_event(path) is None

    assert f"Could not parse status snapshot from {path}" in caplog.text
