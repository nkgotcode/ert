...
from ert.gui.tools.plot.plottery.plots.plot_tools import PlotTools
...
def test_that_tooltip_manager_catches_invalid_axes(plot_data_1D, caplog):
    ...
    event.inaxes = None  # Simulate event occurring outside of axes
    ax.figure.canvas.callbacks.process("motion_notify_event", event)
...
def test_that_tooltip_manager_catches_invalid_event_coordinates(plot_data_1D, caplog):
    caplog.set_level("WARNING")
    x_data, labels = plot_data_1D
